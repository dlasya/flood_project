...
export default function MinimalCityPlanner() {
  console.log('MinimalCityPlanner component mounted!');
  ...
  // Hardcoded test data for immediate testing
  const testIssues = [
    {
  ...
  useEffect(() => {
    ...
    if (selectedState && selectedDistrict) {
      // Start with hardcoded data first
      console.log('Using hardcoded test data');
      setIssues(testIssues);
      console.log('Hardcoded issues loaded:', testIssues.length);
  ...
  const fetchDistrictIssues = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log('API Issues loaded:', data.issues.length);
        alert(`API: Found ${data.issues.length} issues for ${selectedDistrict}`);
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#bbdefb] text-sm">Minimal Version</p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <p className="text-[#64748b]">Selected State</p>
              <p className="font-semibold text-[#0d1b2a]">{selectedState || 'None'}</p>
            </div>
            <div>
              <p className="text-[#64748b]">Selected District</p>
              <p className="font-semibold text-[#0d1b2a]">{selectedDistrict || 'None'}</p>
            </div>
            <div>
              <p className="text-[#64748b]">Issues Count</p>
              <p className="font-semibold text-[#0d1b2a]">{issues.length}</p>
            </div>
            <div>
              <p className="text-[#64748b]">Loading</p>
              <p className="font-semibold text-[#0d1b2a]">{loading ? 'Yes' : 'No'}</p>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-[#e3f2fd]">
              {issues.map((issue, index) => (
                <div key={issue.id || index} className="p-6 hover:bg-[#f8fafc] transition-colors">
                  <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="text-lg font-semibold text-[#0d1b2a] mb-2 capitalize">
                        {issue.issue_type?.replace('_', ' ') || 'Unknown Issue'}
                      </h3>
